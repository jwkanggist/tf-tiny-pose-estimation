...
def train(dataset_train, dataset_valid,train_config,model_config):
    ...
    # -----------------------------------------------------
    pred_heatmap,pred_heatmap_resized = modelbuilder.get_model(model_in=inputs,scope='model')
    # -----------------------------------------------------
    ...
    loss_heatmap_op          = tf.losses.mean_squared_error(labels=true_heatmap,
                                                            predictions=pred_heatmap)
    ...
    tf.logging.info('[Summary] tf.summary is logging on %s' % train_config.tflogdir +'train/')
    tf.logging.info('[Summary] tf.summary is logging on %s' % train_config.tflogdir +'valid/')
